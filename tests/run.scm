...
(test-group "git"
  ...
  (let ((repo (repository-open *repository*)))
    ...
    (test-group "config"
      ...
      (test #t (config? (config-open "now-a-config")))
      (let ((cfg (config-open repo)))
        ...
        (for-each
          (lambda (name value)
            (test-assert (config-set cfg name value)))
          (map car *config*)
          (map cdr *config*))
        ...
        (test "origin" (config-get cfg "branch.master.remote"))
        (test "origin" (config-get cfg "branch.master.remote" 'string))
        (test-error  (config-get cfg "branch.master.remote" 'number))
        (test-error  (config-get cfg "branch.master.remote" 'boolean))
        (test-assert (config-unset cfg "branch.master.remote"))
        (test-error  (config-get cfg "branch.master.remote"))))
                ...
                (let ((t2 (create-tree repo ix)))
                  ...
                  (test-group "tree diffs"
                    ...
                    (let ((diff (tree-diff repo t1 t2)))
                      ...
                      (test 1 (length diff))
                      (let* ((diff (car diff))
                             (old (diff-old-file diff))
                             (new (diff-new-file diff)))
                        (test #t (diff? diff))
                        (test 0  (diff-similarity diff))
                        (test 'added (diff-status diff))
                        (test #t (diff-file? old))
                        (test #t (diff-file? new))
                        (test #t (oid? (diff-file-id old)))
                        (test #t (oid? (diff-file-id new)))
                        (test 0 (diff-file-mode old))
                        (test 33188 (diff-file-mode new))
                        (test (car *files*) (diff-file-path new)))))))))
